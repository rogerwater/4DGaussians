...
class ObservationList:
    ...
    For 'flow' modality: shape is [T, N, 3] where N is number of flow points, 3 is [x, y, visibility]
    """

    POSSIBLE_MODALITIES = ["rgb", "depth", "normal", "policy", "flow"]
    ...
    def __init__(self, data_dict, add_time_dimension=False, image_shape=(64, 64)):
        ...
        for k, v in self.data_dict.items():
            ...
            # Skip resizing for flow modality (not an image)
            if k == "flow":
                continue
            if len(v.shape) > 2:
    ...
    def append(self, *obs, axis=0):
        # 只处理所有ObservationList都有的共同keys
        all_keys = set(self.data_dict.keys())
        for o in obs:
            all_keys = all_keys.intersection(set(o.data_dict.keys()))
        
        for k in all_keys:
            to_cat = [self.data_dict[k]] + [o.data_dict[k] for o in obs]
            self.data_dict[k] = np.concatenate(to_cat, axis=axis)
        
        # Preserve image_shape from first observation in chain
        if obs and hasattr(obs[0], 'image_shape'):
            self.image_shape = obs[0].image_shape
        return self

    def repeat(self, times, axis=0):
        new_dict = {}
        for k, item in self.data_dict.items():
            new_dict[k] = np.repeat(item, times, axis=axis)
        # Preserve image_shape
        return ObservationList(new_dict, image_shape=self.image_shape)
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, slice):
            return ObservationList(
                {k: v[key] for k, v in self.data_dict.items()},
                image_shape=self.image_shape  # Preserve image_shape
            )
        if isinstance(key, str):
            return self.data_dict[key]
        # Note that if we select only a single index, we maintain the time dimension
        return ObservationList(
            {k: v[key][None] for k, v in self.data_dict.items()},
            image_shape=self.image_shape  # Preserve image_shape
        )
    
    def __setitem__(self, key, value):
        """
        Enable item assignment like goal_obs['rgb'] = data
        """
        if not isinstance(key, str):
            raise TypeError("ObservationList keys must be strings")
        if key not in self.POSSIBLE_MODALITIES:
            raise ValueError(f"Key '{key}' not in possible modalities: {self.POSSIBLE_MODALITIES}")
